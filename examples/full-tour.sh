...
if blm_is_tty; then printf 'stdout is a TTY\n'; else printf 'stdout is not a TTY\n'; fi
if blm_color_enabled; then printf 'color output is enabled\n'; else printf 'color output is disabled for this context\n'; fi
...
if blm_require_command __bashloom_example_missing_command__; then exit 1; else printf 'missing command correctly rejected\n'; fi
if blm_require_root; then printf 'running as root\n'; else printf 'running as non-root user\n'; fi
...
if blm_run bash -c 'exit 42'; then exit 1; else status=$?; printf 'preserved exit status: %d\n' "$status"; [[ $status -eq 42 ]]; fi
...
[[ -f $workdir/step-file ]]
if blm_step "Intentional failing step" bash -c 'exit 33'; then exit 1; else [[ $? -eq 33 ]]; fi

section "Retry and polling"
counter="$workdir/retry-counter"
printf '0\n' >"$counter"
flaky_command() { local value; value=$(<"$counter"); value=$((value + 1)); printf '%s\n' "$value" >"$counter"; printf 'attempt %d\n' "$value"; ((value >= 3)); }
blm_retry --attempts 5 --delay 0 --backoff 1 flaky_command
[[ $(<"$counter") -eq 3 ]]
ready_file="$workdir/ready"
( sleep 1; touch "$ready_file" ) >/dev/null 2>&1 &
service_ready() { [[ -f $ready_file ]]; }
blm_wait_for --timeout 5 --interval 1 service_ready
never_ready() { return 1; }
if blm_wait_for --timeout 1 --interval 1 never_ready; then exit 1; else [[ $? -eq 124 ]]; fi

section "Command timeout"
if blm_timeout --timeout 1 --grace 1 sleep 10; then exit 1; else [[ $? -eq 124 ]]; fi
...
blm_ensure_symlink "$real_file" "$link_file"
if blm_ensure_symlink "$other_file" "$link_file"; then exit 1; else printf 'conflicting symlink correctly rejected\n'; fi
config="$workdir/config.ini"
printf 'version=old\n' >"$config"
generate_config() { printf 'version=new\n'; printf 'enabled=true\n'; }
blm_atomic_write "$config" generate_config
[[ $(<"$config") == $'version=new\nenabled=true' ]]
cp "$config" "$workdir/config-before-failure.ini"
failing_generator() { printf 'partial content that must not be installed\n'; return 55; }
if blm_atomic_write "$config" failing_generator; then exit 1; else [[ $? -eq 55 ]]; cmp -s "$config" "$workdir/config-before-failure.ini"; fi
...
if blm_path_is_absolute relative/path; then exit 1; fi
...
printf '%s\n' '# literal data' 'APP=Bashloom' 'PAYLOAD=$(printf not-executed)' >"$runtime_config"
...
BLM_OUTPUT_MODE=plain BLM_LOG_LEVEL=debug BLM_LOG_FILE="$log_file" blm_log debug "debug record"
BLM_OUTPUT_MODE=json BLM_LOG_FILE="$log_file" blm_log info "runtime ready"
[[ -s $log_file ]]
